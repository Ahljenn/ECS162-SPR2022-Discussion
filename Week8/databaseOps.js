...
module.exports = {
  ...
  store_and_get_user: store_and_get_user,
  get_user: get_user
}
...
// Profile table manipulations
const insertUser = "insert into ProfileTable (userid, fname) values (?,?)"
const getUser = "select * from ProfileTable where userid = ?";

async function store_and_get_user(user) {
  let userEntry = await db.get(getUser, [user.userid]);
  if (!userEntry) {
    await db.run(insertUser, [user.userid, user.fname]);
  }
}

async function get_user(userid) {
  return await db.get(getUser, [userid]);
}